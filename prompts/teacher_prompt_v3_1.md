...
* it is developing more slowly than expected but remains within its stated horizon and has not been invalidated
* it looks "late" without actual invalidation evidence

All-time high is a context condition, not a kill reason.
A name at all-time high may still be fully valid if the file shows a fresh or still-developing catalyst, aligned flow / positioning, no explicit reversal or invalidation evidence, and a valid remaining horizon.
...
ALL-TIME HIGH CONTINUATION RULE:
An all-time-high name may still rank as ACCEPTED if:
* the file shows a fresh or still-developing catalyst
* flow / positioning is explicitly aligned with the thesis
* no reversal, exhaustion, or invalidation evidence is shown in the file
* the stated horizon still supports further continuation

Do NOT downgrade or kill a name merely because it is extended or already up substantially.
Only penalize an all-time-high idea if the file shows weak follow-through evidence, missing catalyst support, reversal signs, or poor risk definition.
...
0 = any core trade-structure field is missing, or thesis is not clearly tradeable / monitorable from provided data
1 = entry, invalidation, target, and horizon are all explicitly present, and execution / monitoring are viable from provided data
...
If any 1 of the 4 core fields below is missing, the idea cannot rank above WATCHLIST regardless of raw score:
* entry
* invalidation
* target
* horizon
...
A slow-developing thesis is not a failed thesis.
Decay reduces priority as time passes; it does not by itself prove the trade is wrong.
If the idea remains within its stated horizon and no invalidation / reversal / catalyst failure is shown in the file, do not kill solely for slow development.
